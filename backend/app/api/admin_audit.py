...
async def get_audit_logs(
    ...
    entity_type: Optional[str] = Query(
        None, description="Filter by entity type (e.g. USER, DOCUMENT, CHAT_THREAD)"
    ),
    sort_by: Optional[str] = Query(
        None, description="Sort column (created_at or user_email)"
    ),
    ...
    if entity_type:
        ...
        query = query.where(
            cast(AuditLog.entity_type, String).ilike(f"%{entity_type.upper()}%")
        )
    ...
    return PaginatedAuditLogsResponse(
        data=log_responses, total=total, page=page, limit=limit, total_pages=total_pages
    )
