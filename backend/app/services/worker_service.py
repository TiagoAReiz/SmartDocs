...
class WorkerService:
    ...
    async def _process_next_job(self):
        ...
        async with async_session() as db:
            ...
            logger.info(
                f"[Worker] Picked up job {job.id} for document {job.document_id}"
            )
        ...
        async with async_session() as db:
            ...
            if success:
                ...
                if job.attempts >= self.max_retries:
                    ...
                    job.completed_at = datetime.utcnow()
                    logger.error(
                        f"[Worker] Job {job_id} FAILED permanently after {job.attempts} attempts."
                    )
                    await db.execute(
                        update(Document)
                        .where(Document.id == doc_id)
                        .values(
                            ...
                            error_message="Extração abortada após múltiplas tentativas.",
                        )
                    )
                else:
                    job.status = JobStatus.PENDING  # Queue for retry
                    logger.warning(
                        f"[Worker] Job {job_id} failed. Retrying... (Attempt {job.attempts}/{self.max_retries})"
                    )
